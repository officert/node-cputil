...
var convertStarPrintMarkUp = function (_a) {
    var text = _a.text, variables = _a.variables, printerType = _a.printerType, contentType = _a.contentType;
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    ...
                    fileName = "tmp-".concat(new Date().getTime(), ".png");
                    tmpFilePath = path.join(__dirname, "./tmp/".concat(fileName));
                    outputFilePath = path.join(__dirname, "./output/".concat(fileName.replace('.png', '.bin')));
                    outputFormat = contentType !== null && contentType !== void 0 ? contentType : StarContentType.STAR_PRNT;
                ...
                case 1:
                    // const cmd = `${CPUTIL_PATH} ${printerType} dither scale-to-fit decode ${outputFormat} "${tmpFilePath}" "${outputFilePath}"`
                    _b.sent();
                    return [4 /*yield*/, nodeHtmlToImage({
                            output: tmpFilePath,
                            html: text,
                            content: variables !== null && variables !== void 0 ? variables : {},
                            type: 'png',
                            encoding: 'binary',
                        })];
                case 2:
                    ...
                    return [4 /*yield*/, readFile(tmpFilePath)];
                case 3:
                    fileBuffer = (_b.sent());
                    // await Promise.all([
                    //   // deleteFile(tmpFilePath), deleteFile(outputFilePath)
                    // ])
                    return [2 /*return*/, fileBuffer.toString()];
            }
        });
    });
};
